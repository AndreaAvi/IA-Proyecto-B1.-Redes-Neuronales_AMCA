...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const BACKEND_URL =
    "https://ia-proyecto-b1-redes-neuronales-amca.onrender.com";
...
    };
  ...
  async function handlePredict() {
    ...
    try {
      ...
      const claseStr =
        data.clase === 1 ? "Clase 1 (Rojo)" : "Clase 0 (Amarillo)";
...
        }], layoutLoss);
  ...
  async function updatePlot3D(newPointData = null) {
    try {
      ...
      if (!response.ok)
        throw new Error("No se pudieron cargar los datos de la gráfica.");
      ...
      const unzip = (arr) =>
        arr[0]
          ? [arr.map((p) => p[0]), arr.map((p) => p[1]), arr.map((p) => p[2])]
          : [[], [], []];
      ...
      const traces = [
        {
          ...
          mode: "markers",
          type: "scatter3d",
          name: "Región Amarillo (0)",
          marker: { color: "#FFDDC1", size: 2, opacity: 0.2 },
        },
        {
          x: r1_x,
          y: r1_y,
          ...
          type: "scatter3d",
          name: "Región Rojo (1)",
          marker: { color: "#FF9AA2", size: 2, opacity: 0.2 },
        },
        {
          x: m0_x,
          y: m0_y,
  ...
  socket.on("training_progress", (data) => {
    ...
    if (trainStatus)
      trainStatus.textContent = `Epoch ${data.epoch} / ${
        data.total_epochs
...
        }], layoutLoss);
  ...
  socket.on("training_finished", (data) => {
    ...
    if (trainStatus)
      trainStatus.textContent = `¡Entrenamiento finalizado! Loss final: ${data.final_loss.toFixed(
        6
      )}`;
    if (progressBar) {
